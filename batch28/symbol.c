...
void checkRedec(Tables T, astNode root, int ind) {
	...
	if (strcmp(temp->lex, "106") == 0) {
		if (getType2(T, temp, ind) == 0) {
			printf("ERROR on line %d : Variable %s undeclared before use\n", temp->ti.lineNum, temp->ti.lex);
		}
	}
...
Tables populateSymbolTable(astNode root, Tables T, int ind) {
	...
	if (temp->ti.tkid == TK_FUNID|| temp->ti.tkid == TK_MAIN) {
		...
		while (thc != NULL) {
			...
			if (thc->headChild->nextChild == NULL) {
				if (getType2(T, thc, ind) != 0)
					printf("ERROR on line %d: Redeclaration of variable %s\n", thc->ti.lineNum, thc->ti.lex);
				e.scope = LOCAL;
		...
		if(fun_flag == 0){
			printf("ERROR on line %d: First define the function '%s' before call \n", hc->nextChild->ti.lineNum, hc->nextChild->ti.lex);
		}
	}
	...
	while(hc1 != NULL || hc2 != NULL){
		if(hc1->ti.tkid != TK_ID || hc1->ti.tkid != TK_RECORDID){
			if(hc1->nextChild != NULL)
				...
				if(flag==1){
					printf ("ERROR at line %d: Function returns (null) but returned value is %s of type: %s\n", hc2->ti.lineNum, hc2->ti.lex, reverseEnum(getType(T, hc2)));
					return -1;
				}
				if(flag == 2){
					printf ("ERROR at line %d: Function declaration has input parameter (null) but called with %s of type: %s", hc2->ti.lineNum, hc2->ti.lex, reverseEnum(getType(T, hc2)));
					return -1;
				}
				if(flag == 3){
					printf ("ERROR at line %d: Function declaration has output parameter (null) but function call assigns in %s of type: %s", hc2->ti.lineNum, hc2->ti.lex, reverseEnum(getType(T, hc2)));
					return -1;
				}
			}
		}
		if(getType(T, hc1) == getType(T, hc2)) {
			...
			if(hc2->nextChild != NULL)	
				...
				if(flag==1){
					printf("ERROR at line %d: Function returns %s of type: %s but returned value is (null)\n", hc1->nextChild->ti.lineNum, hc1->nextChild->ti.lex, reverseEnum(hc1->ti.tkid)); 
					return -1;
				}
				if(flag == 2){
					printf ("ERROR at line %d: Function declaration has input parameter %s of type: %s but called with (null)", hc1->nextChild->ti.lineNum, hc1->nextChild->ti.lex, reverseEnum(hc1->ti.tkid));
					return -1;
				}
				if(flag == 3){
					printf ("ERROR at line %d: Function declaration has output parameter %s of type: %s but function call assigns (null)", hc1->nextChild->ti.lineNum, hc1->nextChild->ti.lex, reverseEnum(hc1->ti.tkid));
					return -1;
				}
			}
			continue;
			...
			if(flag==1){
				printf("ERROR at line %d: Function returns %s of type: %s but returned value is %s of type: %s in the function definition\n", hc1->ti.lineNum, hc1->ti.lex, reverseEnum(getType(T, hc1)), hc2->ti.lex, reverseEnum(getType(T,hc2)));
				return -1;
			}
			if(flag==2){
				printf("ERROR at line %d: Function declaration has input parameter %s of type: %s but called with %s of type: %s", hc1->ti.lineNum, hc1->ti.lex, reverseEnum(getType(T, hc1)), hc2->ti.lex, reverseEnum(getType(T,hc2)));
				return -1;
			}
			if(flag==3){
				printf("ERROR at line %d: Function declaration has output parameter %s of type: %s but function call assigns in %s of type: %s", hc1->ti.lineNum, hc1->ti.lex, reverseEnum(getType(T, hc1)), hc2->ti.lex, reverseEnum(getType(T,hc2)));
				return -1;
			}
		}
	}
...
void funInputCheck(Tables T, astNode root, astNode glob_astr)
{
	...
	if(temp->ti.tkid == TK_CALL){
		...
		if(funnode_in->headChild == NULL){
			printf("Line %d: Please check the call for function '%s': Called before  its definition\n", funnode_in->ti.lineNum, funnode_in->ti.lex);
